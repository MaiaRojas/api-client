...
const Api = {
  get: () => fetch(endpoint),
  expressLogin: expressAuthToken => fetch(`${APIClient.getMainURL()}/v2/auth/exchange/${expressAuthToken}`,
    {
      method: 'post',
    }),
  apiCall: (relativeUrl, rawMethod, queryData = {}, bodyData = {}) => makeHeaders()
    .then((callHeaders) => {
      ...
      fetch(
        `${APIClient.getMainURL()}${relativeUrl}${Object.entries(queryData).reduce(
